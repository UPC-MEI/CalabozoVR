...
public class CameraPointerManager : MonoBehaviour
{
    ...
    private readonly string teleportTag = "Telepor";
    private float scaleSize = 0.025f;

    private void Start() {
        GazeManager.Instance.OnGazeSelection += GazeSelection;
        GazeManager.Instance.OnGazeTelepor += GazeTelepor;
    ...
    private void GazeTelepor() {
        _gazedAtObject?.GetComponent<TeleportPoint>()?.OnPointerClicXR();
    }
    ...
    public void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, _maxDistance))
        {
            ...
            if (_gazedAtObject != hit.transform.gameObject)
            {
                ...
                //GPT
                GazeManager.Instance.StartGazeTelepor();
            }

            if(hit.transform.CompareTag(interactableTag)) {
                PointerOnGaze(hit.point);
            } else if(hit.transform.CompareTag(teleportTag)) {
                PointerOnGaze(hit.point);   
            } else {
                 PointerOutGaze();
            }
